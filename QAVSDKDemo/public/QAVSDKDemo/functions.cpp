...
//
//	ht = (int)BmpInfoHead.biHeight;
//	wd = (int)BmpInfoHead.biWidth;
//
...
void mirror_rgb_to_ycc(unsigned char r, unsigned char g, unsigned char b, unsigned char *yp, unsigned char *cb, unsigned char *cr)
{
    int Y, Cb, Cr;
    /*
...
void mirror_ycc_to_rgb(unsigned char y, unsigned char cb, unsigned char cr, unsigned char *r, unsigned char *g, unsigned char *b)
{
    //  fR = Y + 1.402*(Cr-128);
...
void Mirror_BGR24_to_YUV420SP(unsigned char *pBGR24, int wd, int ht, unsigned char* pYUV420, int mode)
{
    ...
    for (i = 0 ; i < (ht >> 1) ; i++)
    {
        ...
        for (j = 0 ; j < (wd >> 1) ; j++)
        {
            ...
            mirror_rgb_to_ycc(r_00, g_00, b_00, &y_00, &cb_00, &cr_00);
            ...
            mirror_rgb_to_ycc(r_01, g_01, b_01, &y_01, &cb_01, &cr_01);
            ...
            mirror_rgb_to_ycc(r_10, g_10, b_10, &y_10, &cb_10, &cr_10);
            ...
            mirror_rgb_to_ycc(r_11, g_11, b_11, &y_11, &cb_11, &cr_11);
...
void Mirror_YUV420SP_to_BGR24(unsigned char *pYUV420, int wd, int ht, unsigned char* pBGR24, int mode)
{
    ...
    for (i = 0 ; i < (ht >> 1) ; i++)
    {
        ...
        for (j = 0 ; j < (wd >> 1) ; j++)
        {
            ...
            mirror_ycc_to_rgb(y_00, cb, cr, &r_00, &g_00, &b_00);
            mirror_ycc_to_rgb(y_01, cb, cr, &r_01, &g_01, &b_01);
            mirror_ycc_to_rgb(y_10, cb, cr, &r_10, &g_10, &b_10);
            mirror_ycc_to_rgb(y_11, cb, cr, &r_11, &g_11, &b_11);
